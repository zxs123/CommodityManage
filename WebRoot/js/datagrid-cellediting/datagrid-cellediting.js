	...
	function editCell(target, param){
		...
		if (input){
            ...
            input.bind('input',function(e){
            	...
            	if(updateRows && inFlag){ 
            		inFlag = false;
            		for(var t=0;t<updateRows.length;t++){
            			if(rows[param.index] == updateRows[t]){   //已存在
            				break;
            			}
            		}
            		//t指向updateRows中需要插入或者替换的元素的索引	 
            		opts.t = t;
            	}
            });
	...
	function enableCellEditing(target){
		...
		if (opts.clickToEdit){        //如果是支持单击可编辑
			opts.onClickCell = function(index, field, value){
				var isSumTab = false;
				if(opts.title == '合同签订情况' || opts.title == '收费情况'){
					isSumTab = true;
				}
				if(!isSumTab || index != 2){
					...
					if(field !='ck'){
						...
						if(editor){
							$(editor.target).bind('blur',function(e){
								...
								opts.renderFlag = true;
								if(opts.t != undefined && opts.t >= 0){
									var rows = dg.datagrid('getRows');
									updateRows[opts.t] = rows[index];
									opts.t = -1;
								}
							});
						}
